...
const mode = process.argv[4] || 'reassurance';
if (!['reassurance', 'insert-r9'].includes(mode)) throw new Error('Unknown reviewed edit');
const insertMode = mode === 'insert-r9';
...
const name = insertMode ? 'parcel-action-context-r9' : 'parcel-context-r8-reassurance-only';
...
// R8b is rejected as a full take. R9 uses only its reviewed pocket prefix,
// before the malformed parcel is exposed; neither failed toss is assembled.
const sources = (insertMode
  ? ['parcel-toss-r8b.mp4', 'parcel-insert-r9.mp4', 'parcel-reassurance-r8.mp4']
  : ['parcel-reassurance-r8.mp4']).map(file => resolve(dir, file));
const hashes = insertMode ? [
...
for (let i = 0; i < sources.length; i++) {
  if (await hash(sources[i]) !== hashes[i]) throw new Error('Inspected source changed');
}
...
for (const [i, source] of sources.entries()) {
  ...
  const insert = insertMode && i === 1;
  if (p.r_frame_rate !== '24/1' || p.width !== (insertMode && i < 2 ? 1916 : 1920) || p.height !== 1080 || Number(p.nb_frames) < (insert ? 90 : 224)) {
...
const frames = insertMode ? 384 : 960;
const duration = frames / 24;
const filter = (insertMode ? [
  '[0:v]trim=start_frame=0:end_frame=96,setpts=PTS-STARTPTS,setsar=1,settb=AVTB[v0]',
  '[1:v]trim=start_frame=24:end_frame=78,setpts=PTS-STARTPTS,pad=1920:1080:2:0,setsar=1,settb=AVTB[v1]',
  '[2:v]trim=start_frame=36:end_frame=90,setpts=PTS-STARTPTS,pad=1920:1080:2:0,setsar=1,settb=AVTB[v2]',
  '[3:v]trim=start_frame=0:end_frame=180,setpts=PTS-STARTPTS,crop=1536:864:192:0,scale=1920:1080:flags=lanczos,setsar=1,settb=AVTB[v3]',
  '[v0][v1][v2][v3]concat=n=4:v=1:a=0[v]',
  '[0:a]atrim=start=0:end=16,asetpts=PTS-STARTPTS[a]',
] : [
...
const cues = readFileSync(resolve(dir, 'magi-award-assembly-v6.vtt'), 'utf8').split(/\r?\n\r?\n/).flatMap(block => {
  ...
  return [`${timestamp(Math.max(0, start))} --> ${timestamp(Math.min(duration, end))}\n${lines.slice(1).join('\n')}`];
});
writeFileSync(resolve(dir, `${name}.vtt`), 'WEBVTT\n\n' + cues.join('\n\n') + '\n');
writeFileSync(resolve(dir, `${name}.json`), JSON.stringify({
  status: insertMode ? 'Local pocket/toss/reassurance audition only; later unwrap/reaction defects remain' : 'Reassurance-only local audition; original toss and unwrap/reaction defects remain; both generated toss takes rejected',
  baseline: 'magi-award-assembly-v6.mp4', baselineHash: baseHash, baselineStartFrame: startFrame,
  fps: 24, frames, replacementBaselineRange: insertMode ? [16038, 16326] : [16262, 16486],
  takes: insertMode ? [
    { file: 'parcel-toss-r8b.mp4', sha256: hashes[0], creation: 'xS4cXwcjfW', range: [24, 78], normalization: '2px pad each side', restriction: 'Pocket prefix only; reject later malformed prop and placement' },
    { file: 'parcel-insert-r9.mp4', sha256: hashes[1], creation: 'rgPW5P2xtc', range: [36, 90], normalization: '2px pad each side, no scaling or retiming' },
    { file: 'parcel-reassurance-r8.mp4', sha256: reassuranceHash, creation: '3zMfvq5REY', range: [0, 180], mandatoryCrop: '1536:864:192:0' },
  ] : [
...
console.log(`${duration}-second ${mode} audition baked and decoded. Rejected tosses excluded; user screening and public film untouched.`);
