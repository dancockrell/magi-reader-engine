...
const [rootArg, parcelHash, reassuranceHash] = process.argv.slice(2);
if (!rootArg || !/^[a-f0-9]{64}$/.test(parcelHash || '') || !/^[a-f0-9]{64}$/.test(reassuranceHash || '')) {
  throw new Error('Production root and both inspected source SHA-256 hashes are required');
}
const root = resolve(rootArg);
const dir = resolve(root, 'production/award-candidate');
const name = 'parcel-context-r8b';
...
const sources = [resolve(dir, 'parcel-toss-r8b.mp4'), resolve(dir, 'parcel-reassurance-r8.mp4')];
...
for (let i = 0; i < sources.length; i++) {
  if (await hash(sources[i]) !== [parcelHash, reassuranceHash][i]) throw new Error('Inspected source changed');
}
...
const filter = [
  ...
  '[before]trim=start_frame=0:end_frame=96,setpts=PTS-STARTPTS,setsar=1,settb=AVTB[v0]',
  '[1:v]trim=start_frame=0:end_frame=224,setpts=PTS-STARTPTS,setsar=1,settb=AVTB[v1]',
  '[2:v]trim=start_frame=0:end_frame=224,setpts=PTS-STARTPTS,crop=1536:864:192:0,scale=1920:1080:flags=lanczos,setsar=1,settb=AVTB[v2]',
...
writeFileSync(resolve(dir, `${name}.json`), JSON.stringify({
  status: 'Local editorial audition only; following unwrap/reaction continuity still requires correction',
  baseline: 'magi-award-assembly-v6.mp4', baselineHash: baseHash, baselineStartFrame: startFrame,
  fps: 24, frames, replacementBaselineRange: [16038, 16486],
  takes: [
    { file: 'parcel-toss-r8b.mp4', sha256: parcelHash, creation: 'xS4cXwcjfW', range: [0, 224] },
...
console.log('40-second parcel audition baked and decoded. User screening and public film untouched.');
