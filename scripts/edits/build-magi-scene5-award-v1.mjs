...
const output = resolve(root, candidate, 'scene5-award-v1');
const fps = 24;
const shots = [
...
const filters = shots.map((s,i) => `[${i}:v]trim=start_frame=${s.in}:end_frame=${s.out},setpts=PTS-STARTPTS,scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih),setsar=1,settb=AVTB[v${i}]`);
