...
describe('the languages the book offers', () => {
  it('are the four it was translated into', () => {
    expect(
      languagesOf(book)
        .map((l) => l.code)
        .sort()
...
describe('a line in the reader’s language', () => {
  ...
  it('is the reader’s language, not the English again', () => {
    const ko = lineTranslation(book, 'ko', 's1', 0, linesOf(book.units[0]).length);
  ...
  it('refuses a list that does not line up rather than showing the wrong sentence', () => {
    const short = { lineTranslations: { s1: { ko: ['하나', '둘'] } } };
    expect(lineTranslation(short, 'ko', 's1', 0, 20)).toBeNull();
    ...
    expect(lineTranslation(short, 'ko', 's1', 0)).toBe('하나');
  });

  it('says nothing rather than something wrong', () => {
    expect(lineTranslation(book, '', 's1', 0)).toBeNull();
    expect(lineTranslation(book, 'ko', 'nope', 0)).toBeNull();
    expect(lineTranslation(book, 'ko', 's1', 9999)).toBeNull();
    expect(lineTranslation({}, 'ko', 's1', 0)).toBeNull();
  });
});

describe('a word and a phrase', () => {
  it('gives a glossed word its meaning', () => {
    expect(wordTranslation(book, 'ko', 'imputation')).toBe('무언의 비난');
    expect(wordTranslation(book, 'ko', 'IMPUTATION'), 'case should not matter').toBe(
      '무언의 비난'
    );
  });
