...
const raw = 'production/magnific/gift-of-the-magi/scene-12/raw/';
const output = resolve(root, 'public/video/films/magi-scene12-v1');
const fps = 24;
const shots = [
  { name: 'The Magi travel', path: raw + '01-magi-travel.mp4', in: 0, out: 211 },
  { name: 'Gold frankincense and myrrh', path: raw + '02-ancient-gifts.mp4', in: 0, out: 211 },
  { name: 'Two foolish wise children', path: raw + '03-couple-gifts.mp4', in: 0, out: 211 },
  { name: 'The exchanged gifts', path: raw + '04-gifts-insert.mp4', in: 0, out: 211 },
  { name: 'Supper together', path: raw + '05-supper.mp4', in: 0, out: 211 },
  { name: 'They are the Magi', path: raw + '06-window-final.mp4', in: 0, out: 210 },
];
...
let cursor = 0; for (const shot of shots) { shot.timelineStart = cursor / fps; cursor += shot.out - shot.in; shot.timelineEnd = cursor / fps; }
const frames = cursor; const duration = frames / fps;
...
const unit = JSON.parse(readFileSync(resolve(root, 'src/books/magi/book.json'), 'utf8')).units[11];
const lines = unit.stanzas.join('\n').split('\n').map((line) => line.replace(/\{([^|}]+)\|[^}]+\}/g, '$1'));
const voices = lines.map((_, i) => `public/magi-audio/n_s12_${i}.mp3`);
let voiceTime = 0; const stamp = (seconds) => new Date(Math.round(seconds * 1000)).toISOString().slice(11, 23);
...
writeFileSync(resolve(root, 'production/magnific/gift-of-the-magi/scene-12/edit-v1.json'), JSON.stringify({ fps, frames, duration, narrationSeconds: voiceTime, shots, voices }, null, 2) + '\n');
console.log(`Scene 12 baked: ${frames} native frames, ${duration.toFixed(3)} seconds; narration ${voiceTime.toFixed(3)} seconds.`);
