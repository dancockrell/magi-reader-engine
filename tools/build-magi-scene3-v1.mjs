...
const raw = 'production/magnific/gift-of-the-magi/scene-3/raw/';
const output = resolve(root, 'public/video/films/magi-scene3-v1');
const fps = 24;
const shots = [
  { name: 'The last coins', path: raw + '01-counting.mp4', in: 0, out: 227 },
  { name: 'Gray cat and gray fence', path: raw + '02-cat-window-r2.mp4', in: 0, out: 227 },
  { name: 'The narrow flat', path: raw + '03-cross-room.mp4', in: 0, out: 227 },
  { name: 'An idea arrives', path: raw + '04-idea-r2.mp4', in: 0, out: 227 },
  { name: 'The pier glass', path: raw + '05-mirror.mp4', in: 0, out: 227 },
  { name: 'First hairpin', path: raw + '06-hairpin.mp4', in: 0, out: 227 },
  { name: 'Hair loosens', path: raw + '07-loosening.mp4', in: 0, out: 227 },
  { name: 'The cascade', path: raw + '08-cascade.mp4', in: 24, out: 241 },
  { name: 'Weighing its value', path: raw + '09-gather.mp4', in: 0, out: 226 },
  { name: 'Resolve', path: raw + '10-resolve.mp4', in: 0, out: 226 },
];
...
const unit=JSON.parse(readFileSync(resolve(root,'src/books/magi/book.json'),'utf8')).units[2];
const lines=unit.stanzas.join('\n').split('\n').map(line=>line.replace(/\{([^|}]+)\|[^}]+\}/g,'$1'));
const voices=lines.map((_,i)=>`public/magi-audio/n_s3_${i}.mp3`); let voiceTime=0; const stamp=s=>new Date(Math.round(s*1000)).toISOString().slice(11,23);
...
writeFileSync(resolve(root,'production/magnific/gift-of-the-magi/scene-3/edit-v1.json'),JSON.stringify({fps,frames,duration,narrationSeconds:voiceTime,shots,voices},null,2)+'\n');
console.log(`Scene 3 baked: ${frames} native frames, ${duration.toFixed(3)} seconds; narration ${voiceTime.toFixed(3)} seconds.`);
