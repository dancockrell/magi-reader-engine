...
const output = resolve(root, 'public/video/films/magi-scene5-v1');
const fps = 24;
const shots = [
  { name: 'Down to the street', path: raw + '01-stairs-down.mp4', in: 0, out: 204 },
  { name: 'The hair-goods shop exterior', path: raw + '02-shop-exterior-r3.mp4', in: 0, out: 204 },
  { name: 'One flight up', path: raw + '03-stairs-up.mp4', in: 0, out: 204 },
  { name: 'Madame Sofronie', path: raw + '04-sofronie-r2.mp4', in: 0, out: 203 },
  { name: 'Twenty dollars', path: raw + '05-inspection.mp4', in: 0, out: 203 },
  { name: 'Give it to me quick', path: raw + '06-cut-r2.mp4', in: 0, out: 203 },
];
...
const unit=JSON.parse(readFileSync(resolve(root,'src/books/magi/book.json'),'utf8')).units[4];
const lines=unit.stanzas.join('\n').split('\n').map(line=>line.replace(/\{([^|}]+)\|[^}]+\}/g,'$1'));
const voices=lines.map((_,i)=>`public/magi-audio/n_s5_${i}.mp3`); let voiceTime=0; const stamp=s=>new Date(Math.round(s*1000)).toISOString().slice(11,23);
...
writeFileSync(resolve(root,'production/magnific/gift-of-the-magi/scene-5/edit-v1.json'),JSON.stringify({fps,frames,duration,narrationSeconds:voiceTime,shots,voices},null,2)+'\n');
console.log(`Scene 5 baked: ${frames} native frames, ${duration.toFixed(3)} seconds; narration ${voiceTime.toFixed(3)} seconds.`);
