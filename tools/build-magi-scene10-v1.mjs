...
const raw = 'production/magnific/gift-of-the-magi/scene-10/raw/';
const output = resolve(root, 'public/video/films/magi-scene10-v1');
const fps = 24;
const shots = [
  { name: 'The parcel', path: raw + '01-parcel.mp4', in: 0, out: 224 },
  { name: 'Nothing could change my love', path: raw + '02-reassurance.mp4', in: 0, out: 224 },
  { name: 'Della unwraps it', path: raw + '03-unwrap.mp4', in: 0, out: 224 },
  { name: 'Joy and tears', path: raw + '04-joy-tears.mp4', in: 0, out: 224 },
  { name: 'The combs', path: raw + '05-combs-reveal.mp4', in: 0, out: 225 },
  { name: 'The Broadway window', path: raw + '06-shop-memory.mp4', in: 0, out: 225 },
  { name: 'Jeweled tortoiseshell', path: raw + '07-combs-macro.mp4', in: 0, out: 224 },
  { name: 'Too short for them', path: raw + '08-mirror-combs.mp4', in: 0, out: 224 },
  { name: 'Jim comforts her', path: raw + '09-comfort.mp4', in: 0, out: 224 },
  { name: 'My hair grows fast', path: raw + '10-hair-grows.mp4', in: 0, out: 224 },
];
...
const unit = JSON.parse(readFileSync(resolve(root, 'src/books/magi/book.json'), 'utf8')).units[9];
const lines = unit.stanzas.join('\n').split('\n').map((line) => line.replace(/\{([^|}]+)\|[^}]+\}/g, '$1'));
const voices = lines.map((_, i) => `public/magi-audio/n_s10_${i}.mp3`);
let voiceTime = 0;
...
writeFileSync(resolve(root, 'production/magnific/gift-of-the-magi/scene-10/edit-v1.json'), JSON.stringify({ fps, frames, duration, narrationSeconds: voiceTime, shots, voices }, null, 2) + '\n');
console.log(`Scene 10 baked: ${frames} native frames, ${duration.toFixed(3)} seconds; narration ${voiceTime.toFixed(3)} seconds.`);
