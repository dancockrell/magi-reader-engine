...
const raw = 'production/magnific/gift-of-the-magi/scene-6/raw/';
const output = resolve(root, 'public/video/films/magi-scene6-v1');
const fps = 24;
const shots = [
  { name: 'Searching the street', path: raw + '01-street-search.mp4', in: 0, out: 228 },
  { name: 'Turning the stores inside out', path: raw + '02-jeweler-search.mp4', in: 0, out: 216 },
  { name: 'Made for Jim', path: raw + '03-chain-reveal.mp4', in: 0, out: 216 },
  { name: 'Quietness and value', path: raw + '04-chain-macro.mp4', in: 0, out: 192 },
  { name: 'Worthy of the watch', path: raw + '05-chain-recognition.mp4', in: 0, out: 216 },
  { name: 'Twenty-one dollars', path: raw + '06-transaction.mp4', in: 0, out: 216 },
  { name: 'Home with eighty-seven cents', path: raw + '07-hurry-home.mp4', in: 0, out: 228 },
  { name: 'The old leather strap', path: raw + '08-chain-and-watch.mp4', in: 0, out: 230 },
];
...
const unit = JSON.parse(readFileSync(resolve(root, 'src/books/magi/book.json'), 'utf8')).units[5];
const lines = unit.stanzas.join('\n').split('\n').map((line) => line.replace(/\{([^|}]+)\|[^}]+\}/g, '$1'));
const voices = lines.map((_, i) => `public/magi-audio/n_s6_${i}.mp3`);
let voiceTime = 0;
...
writeFileSync(
  resolve(root, 'production/magnific/gift-of-the-magi/scene-6/edit-v1.json'),
  JSON.stringify({ fps, frames, duration, narrationSeconds: voiceTime, shots, voices }, null, 2) + '\n'
);
console.log(`Scene 6 baked: ${frames} native frames, ${duration.toFixed(3)} seconds; narration ${voiceTime.toFixed(3)} seconds.`);
