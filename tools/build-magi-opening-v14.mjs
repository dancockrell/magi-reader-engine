...
const output = resolve(root, 'public/video/films/magi-opening-v14');
const title = 'docs/film-edits/opening-v14-title.ass';
const fps = 24;
...
const shots = [
  { name: 'The story opens — music only', path: pickup + 's1-book-opening-v11-raw.mp4', in: 0, out: 30 },
  { name: 'The illustration', path: pickup + 's1-book-opening-v11-raw.mp4', in: 156, out: 216, crop: '1440:810:380:240' },
...
voiceFilters.push(`${voices.map((_, i) => `[a${i}]`).join('')}concat=n=16:v=0:a=1,adelay=3750:all=1[voice]`);
...
writeFileSync(resolve(root, 'production/magnific/gift-of-the-magi/opening-v14-edit.json'), JSON.stringify({
  fps, frames, duration,
  scope: '3.75-second music-only entrance, original narration intact, additional Della coverage; no rejected shop coverage restored',
  narrationOffset: 3.75,
  video: 'public/video/films/magi-opening-v14-silent.mp4',
  preview: 'public/video/films/magi-opening-v14-preview.mp4',
  voiceFiles: voices,
  shots,
}, null, 2) + '\n');
console.log(`Opening v14 baked: ${frames} native frames, ${duration.toFixed(3)} seconds.`);
