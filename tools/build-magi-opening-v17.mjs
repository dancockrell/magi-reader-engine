...
const output = resolve(root, 'public/video/films/magi-opening-v17');
const title = 'docs/film-edits/opening-v17-title.ass';
...
filters.push(`[${bookIndex}:v]trim=start_frame=0:end_frame=240,setpts=PTS-STARTPTS,scale=1920:1080,setsar=1,settb=AVTB[bookClean]`);
// A feathered lighting field, not a rectangular title banner. It clears with the type.
filters.push("color=c=black:s=1920x1080:r=24:d=10,format=rgba,geq=r=0:g=0:b=0:a='195*pow(max(0,1-Y/760),0.7)',fade=t=out:st=5.2:d=1:alpha=1[shade]");
filters.push(`[bookClean][shade]overlay=shortest=1,subtitles=${title}[book]`);
...
writeFileSync(resolve(root, 'production/magnific/gift-of-the-magi/opening-v17-edit.json'), JSON.stringify({
  fps, frames, duration,
  scope: 'Large two-line animated title: staged rise, scale and focus reveal, gold rule, feathered background lighting; book-to-story timing unchanged',
  narrationOffset,
  prologue: { bookIn: 0, bookOut: 240, storyDissolveSeconds: 0.75 },
  video: 'public/video/films/magi-opening-v17-silent.mp4',
  preview: 'public/video/films/magi-opening-v17-preview.mp4',
  voiceFiles: voices,
  shots,
}, null, 2) + '\n');
console.log(`Opening v17 baked: ${frames} native frames, ${duration.toFixed(3)} seconds.`);
