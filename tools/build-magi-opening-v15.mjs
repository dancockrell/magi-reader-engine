...
const output = resolve(root, 'public/video/films/magi-opening-v15');
const title = 'docs/film-edits/opening-v15-title.ass';
const narrationOffset = 15;
const fps = 24;
...
// The prologue is authored separately from the story. No voice enters it.
// Five seconds of pure title, then a one-second reveal into the complete book take.
const bookIndex = shots.length;
filters.push(`color=c=0x0b0c0c:s=1920x1080:r=24:d=6,settb=AVTB[ground]`);
filters.push(`[${bookIndex}:v]trim=start_frame=0:end_frame=240,setpts=PTS-STARTPTS,scale=1920:1080,setsar=1,settb=AVTB[book]`);
filters.push(`[ground][book]xfade=transition=fade:duration=1:offset=5,subtitles=${title},fade=t=out:st=14.25:d=0.75[prologue]`);
filters.push(`[${label}]fade=t=in:st=0:d=0.75[story]`);
filters.push('[prologue][story]concat=n=2:v=1:a=0[titled]');
...
voiceFilters.push(`[17:a]atrim=duration=${duration},asetpts=PTS-STARTPTS,volume='if(lt(t,13),0.24,if(lt(t,16),0.24-0.13*(t-13)/3,0.11))':eval=frame,afade=t=in:d=2,afade=t=out:st=${duration - 2}:d=2[score]`);
...
run(ffmpeg, ['-y', '-v', 'error', '-i', output + '-preview.mp4', '-t', String(narrationOffset), '-c', 'copy', '-movflags', '+faststart', 'public/video/films/magi-prologue-v15.mp4']);
writeFileSync(resolve(root, 'production/magnific/gift-of-the-magi/opening-v15-edit.json'), JSON.stringify({
  fps, frames, duration,
  scope: 'Separate 15-second music-only title and book prologue; gentle dip through black into story; original story edit and voices intact',
  narrationOffset,
  prologue: { titleSeconds: 6, bookIn: 0, bookOut: 240, revealOverlapSeconds: 1, exitFadeSeconds: 0.75 },
  video: 'public/video/films/magi-opening-v15-silent.mp4',
  preview: 'public/video/films/magi-opening-v15-preview.mp4',
  voiceFiles: voices,
  shots,
}, null, 2) + '\n');
console.log(`Opening v15 baked: ${frames} native frames, ${duration.toFixed(3)} seconds.`);
