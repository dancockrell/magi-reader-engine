...
const raw = 'production/magnific/gift-of-the-magi/scene-11/raw/';
const output = resolve(root, 'public/video/films/magi-scene11-v1');
const fps = 24;
const shots = [
  { name: 'Della remembers her gift', path: raw + '01-della-chain.mp4', in: 0, out: 211 },
  { name: 'The platinum chain', path: raw + '02-chain-macro.mp4', in: 0, out: 211 },
  { name: 'Made for Jim', path: raw + '03-chain-in-hand.mp4', in: 0, out: 211 },
  { name: 'Give me your watch', path: raw + '04-asks-watch.mp4', in: 0, out: 211 },
  { name: 'Jim sits and smiles', path: raw + '05-jim-sofa.mp4', in: 0, out: 211 },
  { name: 'The watch is gone', path: raw + '06-empty-pocket.mp4', in: 0, out: 211 },
];
...
const unit = JSON.parse(readFileSync(resolve(root, 'src/books/magi/book.json'), 'utf8')).units[10];
const lines = unit.stanzas.join('\n').split('\n').map((line) => line.replace(/\{([^|}]+)\|[^}]+\}/g, '$1'));
const voices = lines.map((_, i) => `public/magi-audio/n_s11_${i}.mp3`);
...
writeFileSync(resolve(root, 'production/magnific/gift-of-the-magi/scene-11/edit-v1.json'), JSON.stringify({ fps, frames, duration, narrationSeconds: voiceTime, shots, voices }, null, 2) + '\n');
console.log(`Scene 11 baked: ${frames} native frames, ${duration.toFixed(3)} seconds; narration ${voiceTime.toFixed(3)} seconds.`);
